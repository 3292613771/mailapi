...
def load_accounts():
    ...
    if not os.path.exists(ACCOUNTS_FILE):
        ...
        return accounts
    try:
        with open(ACCOUNTS_FILE, "r", encoding="utf-8") as f2:
            for line in f2:
                line = line.strip()
...
def save_links(data):
    with open(LINKS_FILE, "w", encoding="utf-8") as f2:
        json.dump(data, f2, ensure_ascii=False, indent=2)
    try:
        with open("links_backup.json", "w", encoding="utf-8") as f2:
            json.dump(data, f2, ensure_ascii=False, indent=2)
    except Exception as e:
        print(f"备份失败: {e}")
...
def load_used_emails():
    try:
        with open(USED_EMAILS_FILE, "r", encoding="utf-8") as f2:
            return json.load(f2)
    except:
        return {"records": {}}

def save_used_emails(data):
    with open(USED_EMAILS_FILE, "w", encoding="utf-8") as f2:
        json.dump(data, f2, ensure_ascii=False, indent=2)
...
def admin():
    ...
    for link_id, link_info in links.items():
        ...
        link_list += (
            '<tr>'
            + '<td>' + link_id + '</td>'
            + '<td>' + link_info.get('buyer_id', '未知') + '</td>'
            + '<td>' + link_info.get('type', '未知') + '</td>'
            + '<td>' + str(len(link_info['emails'])) + '</td>'
            + '<td>' + link_info['created_at'] + '</td>'
            + '<td>' + link_info['expire_at'] + '</td>'
            + '<td>' + status_text + '</td>'
