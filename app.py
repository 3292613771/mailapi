...
DEFAULT_DAYS = 30
DOMAIN = os.environ.get("DOMAIN", "mailauto.zeabur.app")
...
def load_accounts():
    ...
    if not os.path.exists(ACCOUNTS_FILE):
        print(f"警告: {ACCOUNTS_FILE} 不存在，请创建该文件")
        return accounts
    ...
    try:
        ...
        for part in msg_data:
            if isinstance(part, tuple):
                ...
                from_addr = decode_str(msg.get('From', '未知'))
                subject = decode_str(msg.get('Subject', '无主题'))
                ...
                return (
                    ...
                    '<div class="header">'
                    '<h2>&#128231; ' + email + '</h2>'
                    '<div class="info">发件人：' + from_addr + ' | 主题：' + subject + '</div>'
                    '</div>'
                    '<iframe src="data:text/html;base64,' + html_b64 + '" sandbox="allow-popups allow-same-origin"></iframe>'
...
def admin():
    ...
    html_admin = (
        ...
        '<div class="stats">'
        '<span>总邮箱：<strong>' + str(total) + '</strong></span>'
        '<span>已分配：<strong>' + str(used) + '</strong></span>'
        '<span>可用：<strong>' + str(total - used) + '</strong></span>'
        '</div></div>'
        '<div class="card">'
        '<h2>手动生成链接</h2>'
        '<div class="row">'
        ...
        'html += "<button class=\"copy-btn\" onclick=\"copyText(" + JSON.stringify(data.link_url) + ")\">复制链接</button></div>";'
        'html += "<div style=\"margin-top:8px;color:#999;font-size:13px;\">有效期至：" + data.expire_at + "</div>";'
        'html += "<button onclick=\"copyAll(" + JSON.stringify(data.emails.join(",")) + "," + JSON.stringify(data.link_url) + ")\" style=\"margin-top:12px;padding:8px 20px;background:#667eea;color:white;border:none;border-radius:6px;cursor:pointer;font-size:13px;\">复制全部</button>";'
...
def query_page():
    ...
    return (
        '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>邮箱查询系统</title></head>'
...
def query_mail():
    ...
    mail = result[0]
    return (
        '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>邮件查询结果</title></head>'
        '<body style="font-family: Arial, sans-serif; max-width: 800px; margin: 30px auto; padding: 20px;">'
        '<h2>' + email + ' 的最新邮件</h2>'
        '<div style="border:1px solid #ddd;padding:15px;margin:10px 0;border-radius:8px;">'
        '<b>发件人：</b>' + mail['sender'] + '<br>'
        '<b>主题：</b>' + mail['subject'] + '<br>'
        '<b>时间：</b>' + mail.get('time', '') + '<br><hr>'
        '<div style="margin:10px 0;white-space:pre-wrap;">' + mail['content'][:1000] + '</div><br>'
        '<a href="/view_raw?link_id=' + link_id + '&mail_id=' + mail['mail_id'] + '" target="_blank" style="color:#667eea;">查看完整邮件（含链接）</a>'
        '</div><br>'
        '<a href="/query?link=' + link_id + '" style="color:#667eea;">&#8592; 返回查询页面</a>'
        '</body></html>'
    )
